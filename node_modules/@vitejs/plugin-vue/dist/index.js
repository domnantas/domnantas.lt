...
function setResolvedScript(descriptor, script5, ssr) {
  ;
  (ssr ? ssrCache : clientCache).set(descriptor, script5);
}
...
async function genTemplateCode(descriptor, options, pluginContext, ssr) {
  ...
  if (!template4.lang && !template4.src) {
    return transformTemplateInMain(template4.content, descriptor, options, pluginContext, ssr);
  } else {
    if (template4.src) {
...
async function genScriptCode(descriptor, options, pluginContext, ssr) {
  ...
  let map;
  const script5 = resolveScript(descriptor, options, ssr);
  if (script5) {
